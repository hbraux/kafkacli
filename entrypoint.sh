#!/bin/sh
if [ $# -eq 0 ]; then
  echo "Docker Usage: docker run -it --rm -e KAFKA_SERVER=<SERVER> [--add-host <SERVER>:<IP>] $registry/kafkacli ARGS

kafkacli will then connect to Broker <SERVER>:9092 and Schema Registry <SERVER>:8081 if required (to read avro topics)
